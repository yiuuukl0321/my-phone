...
(function(){
 ...
 /* ===== 8. 干掉键盘上那条 AutoFill 栏 ===== */
(function(){
  var inp = document.getElementById('mIn');
  if (!inp || inp.dataset.ce) return;
  inp.dataset.ce = '1';
  ...
  st.textContent =
    ...
    '#mInCE{flex:1;min-width:0;max-height:104px;overflow-y:auto;padding:8px 14px;' +
    ...
    '#mInCE:empty:before{content:attr(data-ph);color:var(--mute);pointer-events:none}';
  document.head.appendChild(st);

  var d = document.createElement('div');
  d.id = 'mInCE';
  d.setAttribute('contenteditable', 'true');
  d.setAttribute('data-ph', inp.placeholder || '在想什么…');
  d.setAttribute('autocapitalize', 'sentences');
  d.setAttribute('spellcheck', 'false');
  inp.parentNode.insertBefore(d, inp);

  d.addEventListener('input', function(){
    inp.value = d.innerText.replace(/\n$/, '');
  });
